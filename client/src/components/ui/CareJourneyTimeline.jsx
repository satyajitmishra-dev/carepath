  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-sm font-semibold text-emerald-100/50 uppercase tracking-wider">Care Journey</h3>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        {steps.map(({ title, description, icon: Icon }, index) => (
          <motion.div
            ...
            className="rounded-xl border border-emerald-500/20 bg-[#0A140F]/85 p-4"
          >
            <div className="w-9 h-9 rounded-lg bg-emerald-900/40 flex items-center justify-center mb-2.5">
              <Icon className="w-4 h-4 text-emerald-300" />
            </div>
            <p className="text-xs font-bold text-[#E8F8F2] leading-snug">{title}</p>
            <p className="text-[11px] text-emerald-100/50 mt-2 leading-relaxed">{description}</p>
          </motion.div>
        ))}
      </div>
    </motion.div>
